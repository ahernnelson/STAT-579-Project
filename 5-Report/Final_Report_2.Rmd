...
library(scales)
library(ggsci)
all_data <- read.csv("..//Datasets//all_data.csv")
`%!in%` <- Negate(`%in%`)
```
...
## Household Characteristics Analysis {#anchor}
...
### Householder Charactors Analysis

#### Householder's Race  
  
\  
     
Here is the plot for Householder's Race 
...
Here is the plot for Householder of Spanish/Hispanic 
...
Here is the plot for Most Recent Place Householder Lived for 6 Months or More  
...
Here is the plot for Reason for Householder Moving   
...
We run a regression model to select the variable which are significant related with the PQI. We found that the Householder's Race ,Householder of Spanish/Hispanic, Reason for Householder Moving and Age Group are what we can interested in.

We explored the relationship between PQI and these householders characters by using bot-plots and density-plot.  
...
The variables in the data are mostly categorical and we do not have time to explores more efficiently tools to deal with these categorical variables. 
